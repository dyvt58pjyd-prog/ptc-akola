...
export default function BilingualRegisterForm({ batches = [] }: { batches?: any[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result.success) {
      ...
      setError(result.error || "नोंदणी अयशस्वी. / Registration failed.");
    }
    setLoading(false);
  };
  ...
  if (success) {
    return (
      <div className="glass-card" style={{ maxWidth: "600px", margin: "0 auto", textAlign: "center", padding: "4rem 2rem" }}>
        ...
        <h1 className="heading-1" style={{ marginBottom: "0.5rem" }}>Registration Successful!</h1>
        <h2 className="heading-2" style={{ color: "var(--text-muted)", marginBottom: "2rem" }}>नोंदणी यशस्वी झाली!</h2>
        <p style={{ marginBottom: "2rem" }}>Your details have been successfully submitted to the PTC Akola database.</p>
        <button onClick={handleReset} className="btn btn-primary" style={{ padding: "0.75rem 2rem", fontSize: "1.1rem" }}>
          Register Another / आणखी एक नोंदणी करा
        </button>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: "800px", margin: "0 auto" }}>
      <div style={{ textAlign: "center", marginBottom: "2rem" }}>
        ...
        <h2 className="heading-2" style={{ color: "var(--text-muted)", marginBottom: "1rem" }}>नवीन प्रशिक्षणार्थी नोंदणी / Recruit Enrollment</h2>
        <p className="text-muted">
          Please fill in your details accurately. / कृपया आपली माहिती अचूक भरा.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="glass-card">
        ...
        <div className="grid-2">
          ...
          <div className="form-group">
            ...
            <select name="homeDistrict" className="form-select" required>
        ...
        <div style={{ marginTop: "2rem", display: "flex", justifyContent: "flex-end" }}>
          <button type="submit" className="btn btn-accent" style={{ padding: "1rem 2rem", fontSize: "1.1rem" }} disabled={loading}>
            <Save size={20} /> {loading ? "Saving..." : "Submit Registration / नोंदणी पूर्ण करा"}
          </button>
        </div>
      </form>
    </div>
